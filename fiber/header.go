...
func (h ResponseHeader) Get(key string) string {
	return strings.FromBytes(h.ResponseHeader.Peek(key))
}
...
func (h ResponseHeader) Values(key string) []string {
	...
	for i := range byteValues {
		values[i] = strings.FromBytes(byteValues[i])
	}
	return values
}
...
func (h ResponseHeader) Range(f func(key, value string)) {
	h.ResponseHeader.VisitAll(func(key, value []byte) {
		f(strings.FromBytes(key), strings.FromBytes(value))
	})
}
...
func (h RequestHeader) Get(key string) string {
	return strings.FromBytes(h.RequestHeader.Peek(key))
}
